...
ClientSocket::ClientSocket() noexcept:
  s_SSL(nullptr), s_SSLCtx(nullptr), m_Type(SocketAddrType::SockAddrType_IPv4)
{};

ClientSocket::ClientSocket(SocketAddrType type) noexcept:
...
ClientSocket &ClientSocket::acceptAsServer(const int32_t server) {
  ...
  socklen_t len = 0;
  switch (this->m_Type) {
    case SocketAddrType::SockAddrType_IPv4:
...
__SockAddrReturnP ClientSocket::getAddress() {
  ...
  switch (this->m_Type) {
    case SocketAddrType::SockAddrType_IPv4: ret.ipv4 = &this->m_IPv4Addr; break;
    case SocketAddrType::SockAddrType_IPv6: ret.ipv6 = &this->m_IPv6Addr; break;
    default: throw runtime_error(EXCEPT_DEBUG("Invalid address enum"));
  }
  return ret;
}
...
ClientSocket &ClientSocket::connectAsClient(const char *host, const int32_t port) {
  ...
  int32_t type = 0;
  switch (this->m_Type) {
    case SocketAddrType::SockAddrType_IPv4: type = AF_INET; break;
    case SocketAddrType::SockAddrType_IPv6: type = AF_INET6; break;
    default: throw runtime_error(EXCEPT_DEBUG("Invalid address enum"));
  }
...
int32_t ClientSocket::getPort() {
  switch (this->m_Type) {
    case SocketAddrType::SockAddrType_IPv4: return ntohs(this->m_IPv4Addr.sin_port);
    case SocketAddrType::SockAddrType_IPv6: return ntohs(this->m_IPv6Addr.sin6_port);
    default: throw runtime_error(EXCEPT_DEBUG("Invalid address enum"));
  }
}