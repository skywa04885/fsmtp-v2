	...
	CheckSUReturnCode checkSU(const string &hostname, const string &checkAddr) {
		...
		try {
			...
			if ((answer_len = res_nquery(
						...
						sizeof(answer)
					)) < 0) return CheckSUReturnCode::CSC_RECORD;
			...
			if (spf_record.empty()) {
				...
				return CheckSUReturnCode::CSC_RECORD;
			}
			...
			if (spf.getAllowNoQuestionsAsked()) {
				...
				return CheckSUReturnCode::CSC_ALL_ALLOWED;
			}

			if (spf.getDeprecated()) {
				DEBUG_ONLY(logger << "Deprecated features detected" << ENDL);
				return CheckSUReturnCode::CSC_DEPRECATED;
			}
			...
			for (auto &addr : ipv4s) {
				if (addr_compare(checkAddr, addr, AddrType::AT_IPv4)) return CheckSUReturnCode::CSC_ALLOWED;
			}
			...
			if (spf.getMXRecordsAllowed()) {
				...
				for (auto &r : mxRecords) if (addr_compare(checkAddr, r, AddrType::AT_IPv4)) {
					return CheckSUReturnCode::CSC_ALLOWED;
				}
			}
			...
			if (spf.getARecordsAllowed()) {
				...
				// Checks if one of the email addresses matches the
				//  address specified by the app
				for (auto &r : aRecords) if (addr_compare(checkAddr, r, AddrType::AT_IPv4)) {
					return CheckSUReturnCode::CSC_ALLOWED;
				}
			}
			...
			if (allowed_domains.size() > 0) {
				for (auto &domain : allowed_domains) {
					if (checkSU(domain, checkAddr)) return CheckSUReturnCode::CSC_ALLOWED;
				}
			}

			DEBUG_ONLY(logger << checkAddr << " is not an SU for " << hostname << ENDL); 
			return CheckSUReturnCode::CSC_NOT_ALLOWED;
		} catch (const runtime_error &e) {
			DEBUG_ONLY(logger << ERROR << "Could not resolve SPF: " << e.what() << ENDL << CLASSIC);
			return CheckSUReturnCode::CSC_SYSTEM_ERROR;
		} catch (...) {
			return CheckSUReturnCode::CSC_SYSTEM_ERROR;
		}
	}
